...
import { INVALID_DATA, user, userIvalid, passwordIvalid, validUser, notData, NOT_DATA } from './mocks/users.mock';
const { expect } = chai;
...
describe('Users Test', () => {
  ...
  it('O avaliador verificará nao pe possivel fazer o login com dados de email incorretos', async function () {
    sinon.stub(User, 'findOne').resolves(userIvalid as User);    const { status, body } = await chai.request(app).post('/login').send(user);
  ...
  it('O avaliador verificará se é lançado um erro caso a senha tenha o tamanho menor que 6', async function () {
    sinon.stub(User, 'findOne').resolves(passwordIvalid as User); 
  ...
  it('O avaliador verificará se é lançado um erro caso o email nao exista', async function () {
    sinon.stub(User, 'findOne').resolves(notData as User); 
